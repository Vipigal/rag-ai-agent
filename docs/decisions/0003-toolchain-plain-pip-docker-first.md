...
description: The project uses classic pip with a pinned requirements.txt inside a venv (no uv/poetry), Python 3.14 provided by pyenv on the host, and Docker Compose as the official way the system is run.
...
> **Amended by [Decision 0010](/docs/decisions/0010-developer-ux-setup-path.md)**
...
and minimal workflow) and anyone else who must set up and run
...
- **Classic pip + venv, no uv/poetry/conda.** Dependencies are declared
  ...
  lockfile, so pinning is what makes a fresh install reproduce
  the tested one.
...
- **uv / poetry** — better resolvers and real lockfiles, but one more
  tool to know and against the owner's explicit preference
...
- Running the system needs only Docker: clone → `docker compose up` → API on
